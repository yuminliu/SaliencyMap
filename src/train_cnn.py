...
def train_cnn(predictor='GCM',predictand='Amazon', ngcm='all'):
    ...
    if 'GCM' in predictor: # predictor=='GCM':
        datapath = '../data/Climate/GCM/GCMdata/tas/processeddata/32gcms_tas_monthly_1by1_195001-200512_World.npy' # 
    ...
    #gcm_names = np.load('../data/Climate/GCM/GCMdata/tas/processeddata/gcm_names.npy')
...
for predictand in ['Amazon','Congo']:
    for predictor in ['GCM','Reanalysis']:
        print(f"processing predictor={predictor}, predictand={predictand}")
        train_cnn(predictor=predictor, predictand=predictand, ngcm='all')
